...
export default function RentCarModal({ car, onClose, onSuccess }) {
	...
	const uploadFile = async (file, path) => {
		...
		const {
			data: { publicUrl },
		} = supabase.storage.from("rentals").getPublicUrl(filePath);
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			try {
				if (files.uber_file)
					urls.uber = await uploadFile(files.uber_file, "uber");
				if (files.criminal_file)
					urls.criminal = await uploadFile(files.criminal_file, "criminal");
				if (files.cnh_ear_file)
					urls.cnh_ear = await uploadFile(files.cnh_ear_file, "cnh_ear");
				if (files.residence_file)
					urls.residence = await uploadFile(files.residence_file, "residence");
				if (files.sne_file) urls.sne = await uploadFile(files.sne_file, "sne");

				// Inspection Photos
				if (startInspectionFiles.length > 0) {
					const uploadPromises = startInspectionFiles.map((f) =>
						uploadFile(f, "inspections/start"),
					);
		...
		<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
			<div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl w-full max-w-2xl shadow-2xl flex flex-col max-h-[95vh] overflow-hidden">
						...
						{STEPS.map((s, idx) => {
							...
							return (
								<div
									...
									{idx !== 0 && (
										<div
											className={`absolute -left-1/2 top-4 w-full h-[2px] -translate-y-1/2 z-0 ${step > idx ? "bg-accent" : "bg-border-color"}`}></div>
									)}
									<div
										className={`w-8 h-8 rounded-lg flex items-center justify-center z-10 transition-all ${
											isActive
				...
				<div className="p-6 overflow-y-auto flex-1">
					{error && (
						<div className="bg-danger/10 text-danger border border-danger/20 p-3 rounded-xl mb-6 text-sm font-medium">
							{error}
						</div>
					)}
							...
							<div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
								...
								<div className="grid grid-cols-1 gap-4 pr-2">
									...
									{[
										{
											id: "uber_file",
											label: "Print Conta Uber",
											file: files.uber_file,
										},
										{
											id: "criminal_file",
											label: "Ficha Criminal",
											file: files.criminal_file,
										},
										{
											id: "cnh_ear_file",
											label: "CNH com EAR",
											file: files.cnh_ear_file,
										},
										{
											id: "residence_file",
											label: "Comprovante de Residência",
											file: files.residence_file,
										},
										{
											id: "sne_file",
											label: "SNE Digital",
										...
										<div key={f.id} className="space-y-1.5">
											...
											<label
												htmlFor={f.id}
												className={`flex items-center justify-between p-4 border-2 border-dashed rounded-2xl cursor-pointer transition-all ${f.file ? "border-accent bg-accent/5" : "border-border-color hover:border-accent/50"}`}>
												<div className="flex items-center gap-3">
													<Paperclip
														className={`w-5 h-5 ${f.file ? "text-accent" : "text-muted-olive"}`}
													/>
													<span className="text-xs font-bold truncate max-w-[250px]">
														{f.file ? f.file.name : `Anexar ${f.label}`}
													</span>
												</div>
												{f.file && (
													<CheckCircle
								...
								<div className="space-y-4">
									<div className="space-y-2">
										...
										<input
											...
											className="hidden"
											id="start_inspection_files"
										/>
										<label
											htmlFor="start_inspection_files"
											className="flex flex-col items-center justify-center p-8 border-2 border-dashed border-border-color rounded-2xl cursor-pointer hover:border-accent hover:bg-accent/5 transition-all group">
									...
									{startInspectionFiles.length > 0 && (
										<div className="grid grid-cols-3 sm:grid-cols-4 gap-2 mt-4">
											{startInspectionFiles.map((file, idx) => (
												<div
													key={idx}
													className="relative group aspect-square rounded-xl overflow-hidden border border-border-color">
													<img
													...
													<button
														...
														className="absolute top-1 right-1 bg-danger text-white p-1 rounded-md opacity-0 group-hover:opacity-100 transition-opacity">
								...
								<div className="space-y-4 pr-2">
									...
									<div className="bg-bg-main/50 p-4 rounded-2xl border border-border-color space-y-3">
										...
										<div className="grid grid-cols-2 gap-x-4 gap-y-2 text-xs">
											<div>
												<p className="text-muted-olive">Nome:</p>
												<p className="font-bold text-main">
													{formData.client_name}
												</p>
											</div>
											<div>
												<p className="text-muted-olive">CPF/CNPJ:</p>
												<p className="font-bold text-main">
													{formData.client_document}
												</p>
											</div>
											<div>
												<p className="text-muted-olive">WhatsApp:</p>
												<p className="font-bold text-main">
													{formData.client_contact}
												</p>
											</div>
											<div>
												<p className="text-muted-olive">E-mail:</p>
												<p className="font-bold text-main">
													{formData.client_email}
												</p>
											</div>
											<div className="col-span-2">
												<p className="text-muted-olive">Nº CNH:</p>
												<p className="font-bold text-main">
													{formData.client_cnh}
												</p>
											</div>
									...
									<div className="bg-bg-main/50 p-4 rounded-2xl border border-border-color space-y-3">
										...
										<div className="text-xs">
											<p className="font-bold text-main">
												...
												{formData.client_address_number}
												{formData.client_address_complement &&
													` - ${formData.client_address_complement}`}
											</p>
											<p className="text-muted-olive">
												{formData.client_neighborhood} — {formData.client_city}/
												...
												<div className="space-y-2">
													...
													<div className="flex flex-wrap gap-2">
														{[
															...
															{
																val: files.criminal_file,
																label: "Ficha Criminal",
															},
															{ val: files.cnh_ear_file, label: "CNH EAR" },
															{
																val: files.residence_file,
																label: "Compr. Residência",
															},
										...
										<div className="space-y-3">
											<div className="grid grid-cols-1 gap-2">
												{formData.personal_references.map((ref, i) => (
													<div
														key={i}
														className="flex justify-between text-xs border-b border-border-color/50 pb-1 italic">
									...
									<div className="bg-primary/10 p-4 rounded-2xl border border-primary/20 space-y-3">
										...
										<div className="grid grid-cols-2 gap-y-3 text-xs">
											...
											<div>
												...
												<p className="font-bold text-main">
													R$ {formData.unit_price}
												</p>
											</div>
											<div className="text-right">
												<p className="text-muted-olive">Caução:</p>
												<p className="font-bold text-main">
													R$ {formData.security_deposit || "0,00"}
												</p>
											</div>
						...
						<div className="pt-6 mt-6 border-t border-slate-100 dark:border-slate-800 flex justify-between gap-3">
							...
							<button
								...
								{loading ? (
									...
									<>
										<CheckSquareOffset className="w-5 h-5 mr-2" />{" "}
										<span>Confirmar e Iniciar Aluguel</span>
									</>
								) : (
									<>
